...
export function SeoEditor({ page, setting, brief }: { page: PageCatalogEntry; setting: PageSeoSettings | null; brief: PageSeoAIBrief | null }) {
  ...
  return <details className="admin-seo-editor">
    <summary><span><strong>{page.label}</strong><small>{page.route}</small></span><span className="admin-seo-summary-state">{setting?.meta_title || setting?.meta_description ? "Personalizado" : "Usando padrão"}</span><i aria-hidden="true">⌄</i></summary>
    <form action={action} className="admin-form admin-seo-form" onChange={() => setDirty(true)}>
      ...
      <fieldset><legend>Open Graph e compartilhamento</legend><div className="admin-seo-fields"><label>Título Open Graph <input name="ogTitle" value={ogTitle} onChange={(event) => setOgTitle(event.target.value)} maxLength={90} placeholder={title || fallbackTitle} /><small>{ogTitle.length}/90</small></label><label>Descrição Open Graph <textarea name="ogDescription" value={ogDescription} onChange={(event) => setOgDescription(event.target.value)} maxLength={220} placeholder={description || fallbackDescription} /><small>{ogDescription.length}/220</small></label><label>URL da imagem SEO <input name="ogImageUrl" type="url" defaultValue={setting?.og_image_url || ""} maxLength={2048} placeholder="https://…/imagem.webp" /><small>Temporário: URL HTTPS. O padrão global será usado se ficar vazio.</small></label><div className="admin-upload-pending" role="note"><strong>Upload indisponível</strong><span>O armazenamento de imagens ainda não está configurado. Configure o bucket <code>seo-images</code> para habilitar o upload.</span><button type="button" disabled>Enviar JPG, PNG ou WebP · 2 MB</button></div></div></fieldset>
