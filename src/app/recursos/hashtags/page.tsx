...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const metadata = await generatePageMetadata("hashtags", { title, description, alternates: { canonical }, openGraph: { type: "website", locale: "pt_BR", siteName: siteConfig.name, title, description, url: canonical } });
  const robots = metadata.robots && typeof metadata.robots === "object" ? metadata.robots : {};
  return { ...metadata, robots: { ...robots, index: indexable && robots.index !== false } };
}
...
export default async function HashtagResourcePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const [params, config, categories, content] = await Promise.all([searchParams, getHashtagResourceConfig(), listActiveContentCategories(), getPageContent("hashtags")]);
  ...
  return <main className="hashtags-page">
    ...
    <section className="hashtags-hero"><Container><nav className="hashtags-breadcrumb" aria-label="Navegação estrutural"><Link href="/">Início</Link><span>/</span><Link href="/recursos">Recursos</Link><span>/</span><strong>Hashtags</strong></nav><div className="hashtags-hero-grid"><div><span className="hashtags-eyebrow">{content.eyebrow}</span><h1>{content.title}</h1><p>{content.description}</p><div className="hashtags-hero-pills">{splitItems(content.pills).map((item)=><span key={item}>{item}</span>)}</div></div><div className="hashtags-hero-visual" aria-hidden="true"><i>#conteúdo</i><i>#instagram</i><i>#marketing</i><i>#tendências</i><strong>↗</strong></div></div></Container></section>
    <section className="hashtags-content"><Container>
      {!access.available ? <div className="hashtags-state"><span>◌</span><h2>Ferramenta temporariamente indisponível</h2><p>O recurso foi pausado no catálogo administrativo. Volte em outro momento.</p></div> : access.locked ? <div className="hashtags-state is-locked"><span>✦</span><h2>{access.reason === "premium_required" ? "Recurso Premium" : access.reason === "authentication_required" ? "Entre para continuar" : "Acesso controlado"}</h2><p>A ferramenta permanece visível, mas os resultados dependem do nível de acesso configurado.</p>{access.reason === "authentication_required" ? <Link className="button" href="/login">Entrar gratuitamente</Link> : <FeatureInterestButton featureKey="resource_hashtags" source="resources_page" />}</div> : <>
        ...
        <div className="hashtags-method"><strong>{content.method_title}</strong><p>{content.method_text}</p><span>Cache atual: {config.cacheMinutes} minutos · limite: {config.maxItems} hashtags</span></div>
      </>}
    </Container></section>
  </main>;
}
