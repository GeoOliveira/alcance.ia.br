...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { detail } = await readDetail(hashtag);
  ...
  return { title, description, alternates: { canonical }, robots: { index: false, follow: false, googleBot: { index: false, follow: false } }, openGraph: { type: "website", locale: "pt_BR", siteName: siteConfig.name, title, description, url: canonical, images: [{ url: "/og.png", width: 1731, height: 909, alt: `Conteúdos relacionados a #${hashtag}` }] } };
}
  ...
  return <main className="hashtag-detail-page">
    ...
    <section className="hashtag-detail-content"><Container>
      {!access.available || access.locked ? <div className="hashtags-state is-locked"><span>✦</span><h2>Acesso controlado</h2><p>Os detalhes desta hashtag não estão disponíveis para o nível de acesso atual.</p><Link className="button button-secondary" href="/recursos/hashtags">Voltar para hashtags</Link></div> : <>
        ...
        {detail.related.length > 0 && <div className="hashtag-detail-related"><strong>Explore hashtags relacionadas</strong><div>{detail.related.map((related) => <Link href={`/recursos/hashtags/${encodeURIComponent(related)}${category ? `?categoria=${encodeURIComponent(category)}` : ""}`} rel="nofollow" key={related}>#{related}</Link>)}</div></div>}
      </>}
    </Container></section>
  </main>;
}
