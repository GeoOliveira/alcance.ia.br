...
export async function getProductResourcesAdminData() {
  const supabase = await createClient();
  const [features, categories, interest] = await Promise.all([
  ...
  if (features.error || categories.error || interest.error) throw new Error("Não foi possível consultar o catálogo de recursos. A migration local pode ainda não ter sido aplicada.");
  ...
  return { features: features.data || [], categories: categories.data || [], interestCounts };
}
