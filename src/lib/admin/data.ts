...
export async function getProductResourcesAdminData() {
  const supabase = await createClient();
  const [features, categories, interest, dashboardModules, dashboardFlags, hashtagRuns] = await Promise.all([
  ...
  return { features: features.data || [], categories: categories.data || [], interestCounts, dashboardModules: dashboardModules.data || [], dashboardFlags: dashboardFlags.data || [], hashtagRuns: hashtagRuns.data || [] };
}
