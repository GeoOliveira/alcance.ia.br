...
export function enabledAdvancedMetricModules(flags: AdvancedFeatureFlags) {
  return Object.entries(flags).filter(([, enabled]) => enabled).map(([key]) => key).sort();
}
