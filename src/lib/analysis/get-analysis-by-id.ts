...
export const getAnalysisById = cache(async (requestId: string, anonymousSessionId: string | undefined) => {
  ...
  if (result?.profile_data && Array.isArray(result.posts_data) && (result.metrics_version !== ANALYSIS_METRICS_VERSION || !result.calculated_metrics)) {
  ...
  if (result?.id) {
    ...
    if (runtime.config.publicVisibility !== "hidden" && runtime.flags.profileAnalysis) {
      const { data: aiRun } = await admin.from("ai_analysis_runs").select("output_snapshot").eq("analysis_result_id", result.id).eq("status", "completed").order("created_at", { ascending: false }).limit(1).maybeSingle();
      ...
      if (parsedAI.success && hasVisibleFeature) Object.assign(result, { ai_output: filterAIOutputByFeatures(parsedAI.data, runtime.flags, runtime.config), ai_visibility: runtime.config.publicVisibility });
    }
  }
