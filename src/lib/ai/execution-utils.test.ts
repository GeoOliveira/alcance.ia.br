...
describe("AI cache hash and retry", () => {
  it("produces a stable profile hash independent of the analysis request", () => {
    const first = hashAIAnalysisInput(input);
    const anotherRequest = { ...input, analysisContext: { ...input.analysisContext, analysisId: "22222222-2222-4222-8222-222222222222" } };
    expect(first).toMatch(/^[a-f0-9]{64}$/);
    expect(hashAIAnalysisInput(input)).toBe(first);
    expect(hashAIAnalysisInput(anotherRequest)).toBe(first);
    expect(hashAIAnalysisInput({ ...input, metrics: { changed: 1 } })).not.toBe(first);
  });
