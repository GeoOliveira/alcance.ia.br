...
export async function generateAIAnalysisForRequest(requestId: string, options: GenerateOptions = {}) {
  ...
  const runBase = { id: executionId, analysis_result_id: row.id, request_id: requestId, provider: "openai", model, prompt_version: PROFILE_ANALYSIS_PROMPT_VERSION, schema_version: AI_ANALYSIS_SCHEMA_VERSION, metrics_version: row.metrics_version || "legacy", engagement_formula_version: row.engagement_formula_version || "legacy", input_hash: inputHash, input_snapshot: input, source: options.source || "automatic", requested_by: options.requestedBy || null, started_at: new Date().toISOString() };
  const recordTerminalFailure = async (code: "insufficient_confidence" | "daily_limit", message: string) => {
