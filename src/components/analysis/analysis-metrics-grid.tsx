...
export function AnalysisMetricsGrid({ metrics }: { metrics: AnalysisMetrics }) {
  const items = [
    ...
    { label: "Engajamento médio estimado", value: formatPercentage(metrics.estimatedEngagementRate), context: `${metrics.engagementLabel} · ${metrics.validEngagementPosts} posts válidos`, icon: "activity" as const },
    { label: "Engajamento típico estimado", value: formatPercentage(metrics.estimatedTypicalEngagementRate), context: "mediana das interações completas", icon: "activity" as const },
