...
export function AnalysisAIInterpretation({ analysis, visibility }: { analysis: AIProfileAnalysisOutput; visibility: "preview" | "full" }) {
  ...
  return <section className="analysis-ai" id="insights-inteligentes" aria-labelledby="analysis-ai-title">
    ...
    <section className="analysis-ai-subsection" id="insights-pontos-fortes"><span>PONTOS FORTES</span><h3>Pontos fortes</h3>{strengths.length > 0 ? <div className="analysis-ai-grid">{strengths.map((item) => <article key={item.title}><h4>{item.title}</h4><p>{item.explanation}</p></article>)}</div> : <p>Não foram identificados pontos fortes com evidência suficiente nesta amostra.</p>}</section>

    <section className="analysis-ai-subsection" id="insights-oportunidades"><span>OPORTUNIDADES</span><h3>Oportunidades</h3>{opportunities.length > 0 ? <div className="analysis-ai-grid">{opportunities.map((item) => <article key={item.title}><span>{priorityLabels[item.priority]}</span><h4>{item.title}</h4><p>{item.explanation}</p><small>{confidenceLabels[item.confidence]}</small></article>)}</div> : <p>Nenhuma oportunidade atingiu a confiança mínima configurada.</p>}</section>
