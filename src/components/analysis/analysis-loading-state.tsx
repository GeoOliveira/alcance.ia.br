...
export function AnalysisLoadingState({ requestId, initialStage }: { requestId: string; initialStage: AnalysisStage }) {
  const router = useRouter(); const [stage, setStage] = useState(initialStage); const [failed, setFailed] = useState(false);
  useEffect(() => { trackEvent("analysis_processing_viewed", { request_id: requestId, page_path: window.location.pathname });
    let active = true; const process = fetch(`/api/analysis-requests/${requestId}`, { method: "POST" }).then(() => { if (active) router.refresh(); }).catch(() => { if (active) setFailed(true); });
  ...
  return <section className="analysis-loading" aria-live="polite"><div className="analysis-loading-copy"><span className="eyebrow">ANÁLISE EM ANDAMENTO</span><h1>Estamos lendo os sinais públicos do perfil.</h1><p>Você pode manter esta página aberta. O resultado aparecerá assim que os dados forem organizados.</p>{failed && <p className="analysis-inline-error">A conexão oscilou. Continuaremos verificando o estado da análise.</p>}<div className="analysis-real-steps">{stages.slice(0, -1).map((item, index) => <div className={index < current ? "done" : index === current ? "active" : ""} key={item}><span>{index < current ? "✓" : index + 1}</span><div><strong>{analysisStageLabels[item]}</strong><small>{index < current ? "Concluído" : index === current ? "Em andamento" : "Aguardando"}</small></div></div>)}</div></div><div className="analysis-skeleton" aria-hidden="true"><div /><div className="analysis-skeleton-grid"><span /><span /><span /></div><div /><div /></div></section>;
}
