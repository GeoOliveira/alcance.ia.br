...
function DashboardCard({ module, data, requestId }: { module: DashboardModuleRecord; data: DashboardData; requestId: string }) {
  ...
  return <article className={`analysis-dashboard-card${module.preview ? " is-premium" : ""}`} data-module={module.key}>
    ...
    {module.preview ? <div className="analysis-dashboard-premium-preview"><div className="analysis-dashboard-premium-content" aria-hidden="true">{insufficient ? <div className="analysis-dashboard-empty"><strong>Dados insuficientes</strong><span>Este gráfico precisa de mais dados válidos.</span></div> : <div className="analysis-dashboard-chart"><ChartForModule moduleKey={module.key} data={data} requestId={requestId} locked /></div>}</div><div className="analysis-dashboard-premium-lock"><button type="button" onClick={() => trackEvent("analysis_dashboard_premium_clicked", { request_id: requestId, section_id: module.key, cta_location: "executive_dashboard" })}>Premium</button></div></div> : insufficient ? <div className="analysis-dashboard-empty" role="status"><strong>Dados insuficientes</strong><span>Este gráfico será exibido quando houver pelo menos {module.minimumData} {module.minimumData === 1 ? "item válido" : "itens válidos"} na análise.</span></div> : <div className="analysis-dashboard-chart"><ChartForModule moduleKey={module.key} data={data} requestId={requestId} /></div>}
  </article>;
}
