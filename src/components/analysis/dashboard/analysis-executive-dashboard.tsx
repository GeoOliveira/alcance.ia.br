...
function ChartForModule({ moduleKey, data }: { moduleKey: DashboardModuleKey; data: DashboardData }) {
...
function DashboardCard({ module, data, requestId }: { module: DashboardModuleRecord; data: DashboardData; requestId: string }) {
  ...
  return <article className={`analysis-dashboard-card${module.preview ? " is-premium" : ""}`} data-module={module.key}>
    ...
    {module.preview ? <div className="analysis-dashboard-premium-state"><div className="analysis-dashboard-preview-bars" aria-hidden="true"><i /><i /><i /><i /></div><strong>Disponível no Plano Premium</strong><span>Veja esta visualização completa ao liberar o módulo.</span><button type="button" onClick={() => trackEvent("analysis_dashboard_premium_clicked", { request_id: requestId, section_id: module.key, cta_location: "executive_dashboard" })}>Conhecer recurso</button></div> : insufficient ? <div className="analysis-dashboard-empty" role="status"><strong>Dados insuficientes</strong><span>Este gráfico será exibido quando houver pelo menos {module.minimumData} {module.minimumData === 1 ? "item válido" : "itens válidos"} na análise.</span></div> : <div className="analysis-dashboard-chart"><ChartForModule moduleKey={module.key} data={data} /></div>}
  </article>;
}
