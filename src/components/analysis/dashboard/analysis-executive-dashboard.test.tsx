...
describe("AnalysisExecutiveDashboard", () => {
  ...
  it("keeps graph cards white with dark titles", () => { const css = readFileSync(new URL("../../../app/analisar/analysis.css", import.meta.url), "utf8"); expect(css).toContain("background: #fff; box-shadow:"); expect(css).toContain(".analysis-dashboard-card h3 { margin: 0; color: #111827;"); });
  it("uses a blue palette for every column chart", () => { const source = readFileSync(new URL("./analysis-executive-dashboard.tsx", import.meta.url), "utf8"); expect(source.match(/<Bar name=/g)).toHaveLength(5); for (const color of ["#2563eb", "#1d4ed8", "#3b82f6", "#93c5fd"]) expect(source).toContain(`fill="${color}"`); });
