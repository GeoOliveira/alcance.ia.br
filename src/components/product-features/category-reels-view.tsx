...
export async function CategoryReelsView({ searchParams, forcedCategory }: { searchParams: Promise<CategoryReelsSearchParams>; forcedCategory?: string }) {
  const [params, config, content] = await Promise.all([searchParams, getCategoryReelsConfig(), getPageContent("category_reels")]);
  ...
  return <main className="reels-trending-page category-reels-page">
    ...
    <section className="reels-trending-hero category-reels-hero"><Container>
      ...
      <div className="reels-trending-hero-grid"><div><span className="reels-trending-eyebrow">{content.eyebrow}</span><h1>{activeCategory ? `Reels de ${activeCategory.name} para explorar.` : content.title}</h1><p>{activeCategory?.description ?? content.description}</p><div className="reels-trending-pills">{splitItems(content.pills).map((item)=><span key={item}>{item}</span>)}</div></div><div className="reels-trending-visual category-reels-visual" aria-hidden="true"><div><span>14 categorias</span><strong>#</strong><small>ideias para o próximo conteúdo</small></div><i>▶</i><b>EXPLORAR</b></div></div>
    </Container></section>
